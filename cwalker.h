...
int walker_line(char** str)
{
    return walker_untilc(str, '\n') + walker_matchc(str, '\n');
}

int walker_until(char** str, const char* pattern)
{
    auto m = walker_mark(*str);
    while (walker_not(str, pattern)) { }
    return walker_mark_len(*str, m);
}
...
int walker_matchn(char** str, const char* pattern, const int n)
{
    return walker_adv(str, walker_equaln(*str, pattern, n));
}
